    ...
    <nav className={`${navbarBase} ${navbarColor}`}>
      <div className="flex items-center justify-between max-w-7xl mx-auto w-full">
        ...
        <div className="hidden md:flex items-center justify-center">
          <ul className="flex space-x-10 font-medium text-[#EAEAE6] text-center">
            ...
            <li className="relative" ref={dropdownRef}>
              <button
                ...
                className="hover:text-[#CFBE3A] focus:outline-none"
              >
                Services
              </button>
              {dropdownOpen && (
        ...
        <div className="hidden md:flex items-center space-x-4">
          <Link to="/cart" className="relative">
            <svg className="w-6 h-6 text-[#EAEAE6]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          ...
          {user ? (
            <button
              ...
              className="flex items-center gap-1 text-[#EAEAE6] font-semibold hover:text-[#D7CD43] transition"
            >
              <span>{user.name}</span>
            ...
            <Link to="/login">
              <button className="bg-[#CFBE3A] hover:bg-[#b9ae31] text-[#303A40] px-4 py-1.5 rounded">
                Login
              </button>
            </Link>
          )}
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden mt-2 bg-[#4F6866] rounded-b-xl shadow-lg p-4 space-y-3">
          {[
            { path: "/", label: "Home" },
          ...
          {user ? (
            <button
              ...
              className="w-full bg-[#D7CD43] text-[#303A40] py-2 px-4 rounded hover:bg-[#C5BC39]"
            >
              {user.name} ▼
            </button>
          ) : (
            <Link to="/login" onClick={() => setMobileMenuOpen(false)}>
              <button className="w-full bg-[#D7CD43] text-[#303A40] py-2 px-4 rounded hover:bg-[#C5BC39]">
                Login
              </button>
            </Link>
          )}
